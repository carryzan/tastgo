...
import { fetchOfflineRevenueBrowser } from '../_lib/client-queries'
import {
...
interface OfflineRevenueChartProps {
  kitchenId: string
}

export function OfflineRevenueChart({ kitchenId }: OfflineRevenueChartProps) {
  ...
  const { data } = useQuery<DailyRevenueBucket[]>({
    ...
    queryFn: () => fetchOfflineRevenueBrowser(kitchenId),
  })
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 flex flex-col gap-3 h-full">
      ...
      <p className="text-sm text-muted-foreground">
        Total {formatCompact(total)} in offline revenue over the last 7 days.
      </p>
    </div>
  )
}
