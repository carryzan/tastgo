...
import {
  saveInventoryUomConfiguration,
  type UomConversionInput,
...
interface ConversionRow extends UomConversionInput {
  key: string
}

interface InventoryItemUomState {
  storage_uom_id: string | null
}

interface StoredConversion extends UomConversionInput {
  id: string
}
...
const allowKey = (context: ContextKey) =>
  `allow_${context}` as keyof UomConversionInput
const defaultKey = (context: ContextKey) =>
  `is_default_${context}` as keyof UomConversionInput
...
function toConversionInput(row: ConversionRow): UomConversionInput {
  return {
    id: row.id,
    uom_id: row.uom_id,
    ...
    is_default_return: row.is_default_return,
...
export function UOMConfigSheet({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!storageUomId) return setError('Storage UOM is required.')
    ...
    for (const row of rows) {
      ...
      seen.add(row.uom_id)
      if (!row.factor_to_storage || row.factor_to_storage <= 0) {
        return setError('Each factor must be greater than 0.')
      }
      if (row.uom_id === storageUomId && row.factor_to_storage !== 1) {
        return setError('Storage UOM factor must be 1.')
      }
    }

    startTransition(async () => {
      const result = await saveInventoryUomConfiguration({
        kitchen_id: kitchen.id,
        inventory_item_id: itemId,
        storage_uom_id: storageUomId,
        conversions: rows.map(toConversionInput),
      })
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <SheetHeader>
          <SheetTitle>UOM Configuration</SheetTitle>
